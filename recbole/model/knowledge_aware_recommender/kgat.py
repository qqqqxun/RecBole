...
class KGAT(KnowledgeRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.embedding_dim = 128  # or whatever value you choose
        self.queue_size1 = 9680  # Set the size of the queue
        self.queue_size2 = 96800 # Set the size of the queue
        self.queue_size3 = 96800 # Set the size of the queue
        self.register_buffer("queue1", torch.randn(self.queue_size1, self.embedding_dim))
        self.register_buffer("queue2", torch.randn(self.queue_size2, self.embedding_dim))
        self.register_buffer("queue3", torch.randn(self.queue_size3, self.embedding_dim))

        '''self.queue1 = nn.functional.normalize(self.queue1, dim=1)
        self.queue2 = nn.functional.normalize(self.queue2, dim=1)
        self.queue3 = nn.functional.normalize(self.queue3, dim=1)'''
        self.queue_ptr1 = 0  # The pointer that indicates the position to be replaced in the queue
        self.queue_ptr2 = 0
        self.queue_ptr3 = 0
        # MoCo损失权重
        self.temp1 = 1.0
        ...
        self.temp3 = 1.0
        self.moco_v = 0.01
        self.moco_w1 = 0.03736022572122106
        self.moco_w2 = 0.02787495526976582
        self.moco_w3 = 0.0024383946628034506
        self.batch_size = 128

    def update_queue(self, queue, queue_ptr, queue_size, new_element):
        # 确定要更新的实际批次大小
        actual_batch_size = new_element.shape[0]
        # 执行常规更新
        space_left = queue_size - queue_ptr
        actual_batch_size = min(actual_batch_size, space_left)
        queue[queue_ptr:queue_ptr + actual_batch_size, :] = new_element[:actual_batch_size].detach()
        queue_ptr += actual_batch_size
        if queue_ptr == queue_size:
            queue_ptr = 0

        return queue, queue_ptr
    def update_queues(self, cts_embedding_2, e_cts_embedding_2, pos_embeddings):
        self.queue1, self.queue_ptr1 = self.update_queue(self.queue1, self.queue_ptr1, self.queue_size1,
                                                         cts_embedding_2)
        self.queue2, self.queue_ptr2 = self.update_queue(self.queue2, self.queue_ptr2, self.queue_size2,
                                                         e_cts_embedding_2)
        self.queue3, self.queue_ptr3 = self.update_queue(self.queue3, self.queue_ptr3, self.queue_size3, pos_embeddings)
    '''def update_queue(self, queue, new_element):
        # 比较queue和new_element的第一维度
        if queue.shape[0] > new_element.shape[0]:
            # 如果queue的第一维度大于new_element的第一维度，则截取queue
            queue = queue[:new_element.shape[0], :]
        elif queue.shape[0] < new_element.shape[0]:
            # 如果queue的第一维度小于new_element的第一维度，则扩展queue
            queue = torch.cat([queue, torch.zeros(new_element.shape[0] - queue.shape[0], queue.shape[1])], dim=0)

        # 更新queue的内容
        queue[:new_element.shape[0], :] = new_element.detach()
    ...
    def cts_loss(self, z_i, z_j, temp, batch_size): #B * D    B * D
        ...
        negative_samples = sim[mask].reshape(N, -1)   #2B*2B-2
    ...
    def ctm(self, z_i, z_j, queue, temp, batch_size):  # B * D    B * D

        N = 2 * batch_size
        z = torch.cat((z_i, z_j), dim=0)  # 2B * D
        sim1 = torch.mm(z, z.T) / temp  # 2B * 2B

        sim_i_j = torch.diag(sim1, batch_size)  # B*1
        sim_j_i = torch.diag(sim1, -batch_size)  # B*1
        positive_samples = torch.cat((sim_i_j, sim_j_i), dim=0).reshape(N, 1)
        mask = self.mask_correlated_samples(batch_size)
        negative_samples = sim1[mask].reshape(N, -1)  # 2B*2B-2
        labels = torch.zeros(N).to(positive_samples.device).long()
        #labels[:batch_size] = 1
        logits = torch.cat((positive_samples, negative_samples), dim=1)  # N * C
        ...
        sim2 = torch.mm(z, queue.T) / temp
        neg = torch.cat((negative_samples, sim2), dim=1)
        logits2 = torch.cat((positive_samples, neg), dim=1)
        loss2 = self.ce_loss(logits2, labels)
        loss=loss1+loss2
        return loss2

    def ctsl(self, z_i, z_j, queue,temp, batch_size): #B * D    B * D
        l_pos = torch.einsum("nc,nc->n", [z_i, z_j]).unsqueeze(-1)
        l_neg = torch.matmul(z_i, queue.clone().detach().T)
    ...
    def moco_loss(self, z_i, z_j, queue, temp, batch_size):  # B * D    B * D
        q = z_i
        k = z_j
        l_pos = torch.einsum("nc,nc->n", [q, k]).unsqueeze(-1)
        # negative logits: NxK
        #l_neg = torch.einsum("nc,ck->nk", [q, queue.clone().detach().T])
        l_neg = torch.matmul(q, queue.clone().detach().T)
        # logits: Nx(1+K)
        logits = torch.cat([l_pos, l_neg], dim=1)
    ...
    def calculate_loss(self, interaction):
        ...
        cts_embedding_1 = user_all_embeddings_1[torch.tensor(user_rand_samples,dtype=torch.long)]
        cts_embedding_2 = user_all_embeddings_2[torch.tensor(user_rand_samples,dtype=torch.long)]

        e_cts_embedding_1 = entity_all_embeddings_1[torch.tensor(entity_rand_samples,dtype=torch.long)]
        e_cts_embedding_2 = entity_all_embeddings_2[torch.tensor(entity_rand_samples,dtype=torch.long)]
        ...
        moco_loss1 = self.ctm(cts_embedding_1, cts_embedding_2, queue=self.queue1, temp=self.temp1,
                                    batch_size=cts_embedding_1.shape[0])
        moco_loss2 = self.ctm(e_cts_embedding_1, e_cts_embedding_2, queue=self.queue2, temp=self.temp2,
                                    batch_size=e_cts_embedding_1.shape[0])
        moco_loss3 = self.ctm(u_embeddings, pos_embeddings, queue = self.queue3, temp = self.temp3,
                                    batch_size = u_embeddings.shape[0])

        '''mts_loss = self.moco_loss(cts_embedding_1, cts_embedding_2, queue=cts_embedding_2, temp=1.0,
                                    batch_size=cts_embedding_1.shape[0])'''
...
#        print("cts_loss:", cts_loss, e_cts_loss, ui_cts_loss)
        loss = mf_loss + self.reg_weight * reg_loss + 0.08 * (moco_loss1 + moco_loss2 + moco_loss3)
        #print(moco_loss1,moco_loss2,moco_loss3)
        #loss = mf_loss + self.reg_weight * reg_loss + 0.01 * (cts_loss + e_cts_loss + ui_cts_loss)

        return loss
        self.update_queues(cts_embedding_2, e_cts_embedding_2, pos_embeddings)
    ...
    def update_attentive_A(self):
        ...
        for rel_idx in range(1, self.n_relations, 1):
            ...
            # Add Gaussian noise to kg_score
            #kg_score += torch.randn_like(kg_score) * self.noise_weight
        ...
        #indices_1 = indices.view(-1, 2)[torch.tensor(drop_edge_1, dtype=torch.long)].view(2, -1)
        indices_1 = (indices.T)[torch.tensor(drop_edge_1, dtype=torch.long)].T
        kg_score_1 = kg_score[torch.tensor(drop_edge_1, dtype=torch.long)]
        ...
        #indices_2 = indices.view(-1, 2)[torch.tensor(drop_edge_2, dtype=torch.long)].view(2, -1)
        indices_2 = (indices.T)[torch.tensor(drop_edge_2, dtype=torch.long)].T
        kg_score_2 = kg_score[torch.tensor(drop_edge_2, dtype=torch.long)]
